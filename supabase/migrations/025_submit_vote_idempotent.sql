-- 025_submit_vote_idempotent.sql
-- Return existing results when user already voted (no duplicate INSERT / coins)
...
BEGIN
  ...
  -- Idempotent: already voted on this question — return live results, no side effects
  IF EXISTS (
    SELECT 1 FROM votes
